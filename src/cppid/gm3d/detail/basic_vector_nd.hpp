...
: public basic_point_vector<basic_vector_nd<T, N>, T, N> {
  ...
  using basic_point_vector<basic_vector_nd<T, N>, T, N>::basic_point_vector;

  constexpr basic_vector_nd() noexcept = default;

  template<typename U,
           std::enable_if_t<
            detail::is_explicit_constructible_v<scalar_type, U>>* = nullptr>
  explicit constexpr basic_vector_nd(basic_vector_nd<U, dimensions> v) noexcept
  ...
  template<typename U,
           ...
           std::enable_if_t<
            ...
            detail::is_explicit_constructible_v<scalar_type, U>>* = nullptr>
  constexpr basic_vector_nd(basic_vector_nd<U, dimensions> v,
  ...
  constexpr basic_vector_nd(basic_vector_nd<U, dimensions> v) noexcept
  : basic_vector_nd{v, std::make_index_sequence<dimensions>()}
  {
  }
  ...
  constexpr auto operator=(const basic_vector_nd<U, dimensions> rhs) noexcept
   -> basic_vector_nd&
  {
  ...
  constexpr auto operator+=(const basic_vector_nd& rhs) noexcept
   -> basic_vector_nd&
  {
  ...
  constexpr auto operator+=(scalar_type rhs) noexcept -> basic_vector_nd&
  {
  ...
  constexpr auto operator==(const basic_vector_nd& rhs) const noexcept -> bool
  {
  ...
  constexpr auto dot(const basic_vector_nd& other) const noexcept -> scalar_type
  {
    return dot(other, std::make_index_sequence<dimensions>());
  }

  constexpr auto magnitude() const noexcept -> scalar_type
  {
    return std::sqrt(dot(*this));
  }

  constexpr auto magnitude_squared() const noexcept -> scalar_type
  {
    return dot(*this);
  }

  constexpr auto to_unit() const noexcept -> basic_vector_nd
  {
    return *this / magnitude();
  }

  constexpr auto reflect(const basic_vector_nd& n) const noexcept
   -> basic_vector_nd
  {
  ...
  constexpr auto cross(const basic_vector_nd& other) const noexcept
   -> basic_vector_nd
  {
    return boost::qvm::cross(*this, other);
  }

  friend constexpr auto operator+(const basic_vector_nd& val) noexcept
   -> basic_vector_nd
  {
    return val;
  }
  ...
  operator+(const basic_vector_nd& lhs,
  ...
  friend constexpr auto operator+(const basic_vector_nd& lhs,
                                  scalar_type rhs) noexcept -> basic_vector_nd
  {
    return lhs + basic_vector_nd{rhs};
  }

  friend constexpr auto operator+(scalar_type lhs,
  ...
  friend constexpr auto operator-(const basic_vector_nd& lhs) noexcept
   -> basic_vector_nd
  {
    return invoke_op{}(
...
private:
  ...
  template<std::size_t... Ns>
  constexpr auto dot(const basic_vector_nd& other,
                     std::index_sequence<Ns...>) const noexcept -> scalar_type
  {
